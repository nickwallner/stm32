...
the remaining 400 interrupts in the nvic are unused. each register is
a group of 32 32-bit register, where each register
register handles 32 interrupts, so we only need to use the first 3
registers from each register, the rest remain reserved (unused) */
typedef struct
{
 ...
 * 
 * @param IQRn Selected IRQ
 */
void NVIC_EnableIRQ(IRQn_Type IQRn);
